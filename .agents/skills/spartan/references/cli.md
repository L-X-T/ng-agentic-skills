...
- **Nx** (`nx.json` present): `pnpm exec nx g @spartan-ng/cli:<generator>` (or `pnpm nx g ...`).
- **Angular CLI** (`angular.json`, no `nx.json`): `ng g @spartan-ng/cli:<generator>`.

Install the plugin first: `pnpm add -D @spartan-ng/cli`.
...
```bash
pnpm exec nx g @spartan-ng/cli:ui                 # interactive multiselect (choose 'all' or specific)
pnpm exec nx g @spartan-ng/cli:ui --name=dialog   # add a single component
...
```bash
pnpm exec nx g @spartan-ng/cli:healthcheck            # report only
pnpm exec nx g @spartan-ng/cli:healthcheck --autoFix  # apply fixes
...
```bash
pnpm exec nx g @spartan-ng/cli:migrate-helm-imports
```
...
pnpm exec nx g @spartan-ng/cli:init
pnpm exec nx g @spartan-ng/cli:ui-theme
pnpm exec nx g @spartan-ng/cli:ui --name=button
```

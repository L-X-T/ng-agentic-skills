...
license: MIT
metadata:
  upstream: spartan-ng/spartan (see references/LICENSE.txt)
user-invocable: false
allowed-tools:
  - Bash(pnpm exec nx g @spartan-ng/cli:*)
...
- **Nx workspace** (has `nx.json`): `pnpm exec nx g @spartan-ng/cli:<generator>` (or `pnpm nx g ...`).
...
```bash
pnpm exec nx g @spartan-ng/cli:info --json     # Nx
...
first component with `ui` (see `references/cli.md`).
...
1. **Use existing components first.** Check `installedComponents`, then `availableComponents`. Find
   ...
   `https://www.spartan.ng/components/<name>`. See `references/mcp.md`.
...
4. **Use semantic colors, never raw values.** `bg-primary text-primary-foreground`, not
   `bg-blue-500`. See `references/styling.md`.

## Critical rules

Read the rule file before doing the related work:

- **`references/styling.md`** - the `hlm()` util, semantic color tokens, layout-only classes,
  `gap-*` over `space-*`, `size-*`, dark mode, no manual z-index on overlays.
- **`references/forms.md`** - compose forms with `hlmField` (label, control, error, description) and
  `hlmFieldSet` / `hlmFieldLegend` on native `<fieldset>`/`<legend>`; option sets (2-7 choices) use
  `hlm-toggle-group`.
- **`references/composition.md`** - items belong inside their group; dialogs/sheets need a title;
...
- **`references/icons.md`** - icons are `<ng-icon name="lucide...">`; register with `provideIcons`;
  no manual sizing classes inside components - use the `size` input.
- **`references/brain-vs-helm.md`** - the two-layer model (one headless library, Brain, plus the styled
  Helm layer); when to reach for Brain directly vs Helm, and how composition works via directives.
- **`references/cli.md`** - every generator (`init`, `ui`, `ui-theme`, `healthcheck`, `info`, `migrate-*`),
  with Nx and Angular CLI invocations.
- **`references/registry.md`** - the Brain-npm + Helm-copy-in distribution model, `components.json`, and the
  fixed component catalog (the shipped CLI uses no remote or custom registry).
- **`references/customization.md`** - theming via the `hlm-tailwind-preset.css`, CSS variables, the `ui-theme`
  generator, and extending copied Helm components.
- **`references/mcp.md`** - using the `@spartan-ng/mcp` tools, resources, and prompts for discovery.
...
3. **Find the component.** Use the MCP tools or `https://www.spartan.ng/components/<name>` for the
   API and examples (`references/mcp.md`). Never guess selectors - confirm them.
4. **Add it.** `pnpm exec nx g @spartan-ng/cli:ui --name=<component>` (Nx) or
...
pnpm exec nx g @spartan-ng/cli:init
ng g @spartan-ng/cli:init

# Project context as JSON
pnpm exec nx g @spartan-ng/cli:info --json

# Add components (interactive, or pass --name)
pnpm exec nx g @spartan-ng/cli:ui
pnpm exec nx g @spartan-ng/cli:ui --name=dialog

# Generate theme variables
pnpm exec nx g @spartan-ng/cli:ui-theme

# Scan + auto-fix deprecated APIs/imports after an upgrade
pnpm exec nx g @spartan-ng/cli:healthcheck --autoFix
```

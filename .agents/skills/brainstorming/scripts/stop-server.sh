...
if [[ -f "$PID_FILE" ]]; then
  ...
  # Only delete ephemeral temp directories - canonicalize first so paths like
  # /tmp/../home/user or symlinks cannot widen the rm -rf target
  tmp_root="$(cd /tmp && pwd -P)"
  session_dir_real="$(cd "$SESSION_DIR" 2>/dev/null && pwd -P || true)"
  if [[ -n "$session_dir_real" && "$session_dir_real" == "$tmp_root"/* ]]; then

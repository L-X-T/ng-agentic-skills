...
function brandMarkup() {
  ...
  // Remote logo is opt-in (SUPERPOWERS_BRANDING=1): loading it pings a third-party
  // host with the version string, which workshop machines should not do by default.
  const logo =
    isTruthyEnv(process.env.SUPERPOWERS_BRANDING) && !SUPERPOWERS_TELEMETRY_DISABLED

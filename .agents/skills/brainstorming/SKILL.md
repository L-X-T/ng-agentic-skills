...
- Do not stage or commit the design document unless the user explicitly asks
...
- Create a detailed implementation plan. If a planning skill exists, use it; otherwise write the plan in the current thread.
- Do NOT invoke implementation skills yet. Planning is the next step.
...
The companion serves everything locally (127.0.0.1). Its upstream branding logo is a remote image and stays **off** unless `SUPERPOWERS_BRANDING=1` is set; setting `SUPERPOWERS_DISABLE_TELEMETRY`, `DISABLE_TELEMETRY`, or `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` also keeps all non-essential network traffic disabled.
...
`.agents/skills/brainstorming/references/visual-companion.md`

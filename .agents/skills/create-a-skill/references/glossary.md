...
The domain model for what makes a skill great. A skill exists to wrangle determinism out of a stochastic system; every term below is a lever on that goal. This is the disclosed reference for [`create-a-skill`](../SKILL.md).
...
A skill the harness may select implicitly when a request matches its **description**. The human may also invoke it explicitly. The host controls discovery and invocation policy; keeping a description does not by itself guarantee selection.

_Avoid_: ability, tool, capability

### User-Invoked

A skill intended for explicit invocation by the human. Keep its **description** and configure the host to disable implicit invocation. Metadata and context costs differ by harness; explicit-only use does not promise zero context load.

_Avoid_: procedure, workflow, command

### Description

Required portable metadata describing what the skill does and when it applies. It supports discovery and selection. Invocation policy is separate; deleting a description can make the skill invalid.
...
The context consumed by discovered skill metadata and any subsequently loaded instructions or references. Measure the host behavior instead of assuming every description is always loaded in full.
...
A skill that points to a small set of related skills and explains when to use each. Respect the invocation policy and authorization of each destination; routing does not grant additional authority.
...
Material outside the skill entry point that multiple skills can read when needed. A shared convention or API reference need not become an independently invocable skill.

...
Use these only to maintain or integrate with an existing Reactive Forms area. For new form work,
prefer Signal Forms – see the [SKILL router](../SKILL.md). Every component below is `OnPush`.
...
export function forbiddenValue(forbidden: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    return control.value === forbidden ? { forbiddenValue: { value: control.value } } : null;
  };
}
